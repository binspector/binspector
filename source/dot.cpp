...
inline bool operator<(const dot_edge_t& x, const dot_edge_t& y) {
    return x.from_m < y.from_m || (x.from_m == y.from_m && x.to_m < y.to_m) ||
...
void graph_struct(const binspector_analyzer_t::structure_map_t& struct_map,
    ...
    for (; iter != last; ++iter) {
        ...
        if (field_type == value_field_type_struct || field_type == value_field_type_enumerated ||
            ...
            conditional_expression_t conditional_type(
                has_conditional_type ?
                    dvalue<conditional_expression_t>(field, key_field_conditional_type) :
                    none_k);
...
std::string format_attribute_map(const dot_attribute_map_t& map) {
    ...
    if (!map.empty()) {
        ...
        for (dot_attribute_map_t::const_iterator iter(map.begin()), last(map.end()); iter != last;
             ++iter) {
...
std::string format_edge(const dot_edge_t& edge) {
    ...
    return result +=
           edge.from_m + " -> " + edge.to_m + format_attribute_map(edge.attribute_map_m) + ";\n";
}
...
void format_graph(dot_graph_t& graph, std::ostream& output) {
    ...
    graph.node_vector_m.erase(adobe::unique(graph.node_vector_m, id_equal_t()),
                              graph.node_vector_m.end());
    ...
    for (dot_cluster_vector_t::iterator iter(graph.cluster_vector_m.begin()),
         last(graph.cluster_vector_m.end());
         iter != last;
         ++iter) {
        ...
        cluster_node_vector.erase(adobe::unique(cluster_node_vector, id_equal_t()),
                                  cluster_node_vector.end());

        for (dot_node_vector_t::const_iterator iter(cluster_node_vector.begin()),
             last(cluster_node_vector.end());
             iter != last;
             ++iter)
    ...
    for (dot_edge_set_t::const_iterator iter(graph.edge_set_m.begin()),
         last(graph.edge_set_m.end());
         iter != last;
         ++iter)
...
void dot_graph(const binspector_analyzer_t::structure_map_t& struct_map,
    ...
    graph_struct(
        struct_map, starting_name, starting_id, binspector_analyzer_t::typedef_map_t(), 0, graph);
