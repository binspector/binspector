...
bool binspector_analyzer_t::jump_into_structure(const adobe::dictionary_t& field,
                                                inspection_branch_t        parent)
{
    return jump_into_structure(get_value(field, key_named_type_name).cast<adobe::name_t>(),
                               parent);
}
...
try
{
    ...
    for (structure_type::const_iterator iter(structure.begin()), last(structure.end()); iter != last; ++iter)
    {
        ...
        else if (type == value_field_type_enumerated)
        {
            ...
            value = finalize_lookup<adobe::any_regular_t>(forest_m->begin(), atom, input_m, true);
            }
        ...
        else if (type == value_field_type_sentry)
        {
            ...
            temp_assignment<std::string>        sentry_name_holder(current_sentry_set_path_m, last_path());
            ...
            {
                error_m << "WARNING: After "
                        << current_sentry_set_path_m
                        << " sentry, read position should be "
                        << sentry_position
                        << " but instead is "
                        << input_m.pos()
                        << ".";
        ...
        try
        {
            ...
            {
                branch_data.expression_m = value_for<adobe::array_t>(field, key_field_assign_expression);
            ...
            const adobe::array_t& field_size_expression(value_for<adobe::array_t>(field, key_field_size_expression));
                ...
                if (has_size_expression)
                {
                    ...
                    if (field_size_type == field_size_delimiter_k)
                    {
                        ...
                        */
                        ...
                        boost::uint64_t     running_stop_value_mask(delimiter_bit_count == 8  ? 0xff :
                                                                    delimiter_bit_count == 16 ? 0xffff :
                                                                    delimiter_bit_count == 32 ? 0xffffffff :
                                                                                                -1);
                    ...
                    else if (field_size_type == field_size_terminator_k)
                    {
                        ...
                        boost::uint64_t running_count(0);
                        boost::uint64_t terminator_mask(read_size == 1 ? 0xff :
                                                        read_size == 2 ? 0xffff :
                                                        read_size == 4 ? 0xffffffff :
                                                                         -1);
    
                        if (read_size_leftovers)
                            throw std::runtime_error("Use of non-byte-aligned fields with a terminator not supported.");
                        ...
                        while (true)
                        {
                            ...
                            boost::uint64_t* termptr(reinterpret_cast<boost::uint64_t*>(&raw[0]));
